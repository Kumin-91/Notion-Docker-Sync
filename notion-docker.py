...
#노션 API 토큰과 데이터베이스 ID를 환경 변수에서 불러오기
def load_config():
    ...
    for key in REQUIRED_KEYS:
        value = getenv(key)
        if value is None:
            raise EnvironmentError(f"[Fatal] Required {key} not found in {ENV_PATH}. Please check your environment variables.")
        config[key] = value
    return config
#환경 변수에서 설정 불러오기
config = load_config()
...
'''
DB = {
        "Docker" : config['NOTION_DOCKER_DB_ID'],
        "Jenkins" : config['NOTION_JENKINS_DB_ID'],
        "Games" : config['NOTION_GAMES_DB_ID']
    }
DEFAULT_DB = config['NOTION_DEFAULT_DB_NAME']
'''
